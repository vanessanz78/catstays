...
test('room ids are unique across primary and per-cat assignments', () => {
  ...
  assert.deepEqual(bookingRoomIds(booking({
    booking_cat_rooms: [
      { cat: { id: 'cat-1', name: 'Car' }, room: { id: 'room-1', name: 'Private 1', type: 'private', price_per_night: 20 } },
      { cat: { id: 'cat-2', name: 'Milo' }, room: { id: 'room-2', name: 'Private 2', type: 'private', price_per_night: 20 } },
    ],
  })), ['room-1', 'room-2']);
});
...
test('conflict detection ignores cancelled and currently dragged bookings', () => {
  ...
  assert.equal(roomHasBookingConflict(bookings, 'room-1', '2026-09-03', '2026-09-04'), true);
  assert.equal(roomHasBookingConflict(bookings, 'room-1', '2026-09-04', '2026-09-05'), false);
  assert.equal(roomHasBookingConflict(bookings, 'room-1', '2026-09-01', '2026-09-03', 'existing'), false);
});
