...
const project='iwyoezwqorddkmqnjbif',version='20260904004000',mode=process.argv[2];
...
try{
 const migration=readFileSync(`supabase/migrations/${version}_verified_sync_snapshots.sql`,'utf8');
 const test=readFileSync('supabase/tests/verified_sync_snapshots_rollback.sql','utf8');
 if(mode==='rehearse')console.log(JSON.stringify(await sql(`begin; ${migration}\n${test.replace(/^begin;\s*$/m,'')}`)));
  ...
  await sql(`begin; ${migration}\ninsert into supabase_migrations.schema_migrations(version,name,statements) values('${version}','verified_sync_snapshots',ARRAY[$migration$${migration}$migration$]);commit;`);
