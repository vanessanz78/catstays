...
const project='iwyoezwqorddkmqnjbif',version='20260904003000',mode=process.argv[2];
...
try{
 const migration=readFileSync(`supabase/migrations/${version}_operational_manual_sync.sql`,'utf8');
 const test=readFileSync('supabase/tests/operational_manual_sync_rollback.sql','utf8');
 if(mode==='rehearse')console.log(JSON.stringify(await sql(`begin; ${migration}\n${test.replace(/^begin;\s*$/m,'')}`)));
  ...
  await sql(`begin; ${migration}\ninsert into supabase_migrations.schema_migrations(version,name,statements) values('${version}','operational_manual_sync',ARRAY[$migration$${migration}$migration$]);commit;`);
